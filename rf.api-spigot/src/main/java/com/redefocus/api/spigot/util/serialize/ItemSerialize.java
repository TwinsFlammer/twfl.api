...
//    private static ItemReflection ItemReflection;
//
//    static {
//        ItemReflection = SpigotAPI.getInstance().getItemReflection();
//    }
    ...
    public static String toBase64(ItemStack itemStack) {
        ...
        try {
            Object nbtTagListItems = ItemReflection.getClass_NBTTagList().newInstance();
            Object nbtTagCompoundItem = ItemReflection.getClass_NBTTagCompound().newInstance();

            Object nms = ItemReflection.getMethod_asNMSCopy().invoke(null, itemStack);

            ItemReflection.getMethod_SaveItem().invoke(nms, nbtTagCompoundItem);

            ItemReflection.getMethod_Add().invoke(nbtTagListItems, nbtTagCompoundItem);

            ItemReflection.getMethod_Save().invoke(null, nbtTagCompoundItem, dataOutput);
        } catch (Throwable ex) {
            ex.printStackTrace();
        }
    ...
    public static ItemStack fromBase64(String data) {
        ...
        try {
            nbtTagCompoundRoot = ItemReflection.getMethod_A().invoke(null, new DataInputStream(inputStream));
            if (nbtTagCompoundRoot != null) {
                nmsItem = ItemReflection.getMethod_CreateStack().invoke(null, nbtTagCompoundRoot);
            }

            toReturn = ItemReflection.getMethod_AsBukkitCopy().invoke(null, nmsItem);
        } catch (Throwable ex) {
            ex.printStackTrace();
        }
...
class ItemReflection {
    ...
    private static Class<?> class_ItemStack, class_NBTBase, class_NBTTagCompound, class_NBTTagList, class_CraftItemStack, class_NBTCompressedStreamTools, class_EntityArmorStand, class_PacketPlayOutSpawnEntityLiving, class_EntityLiving, class_PlayerConnection, class_Packet, class_PacketPlayOutEntityDestroy, class_World, class_CraftWorld, class_Entity;
    private static Method method_asNMSCopy, method_SaveItem, method_Add, method_Save, method_A, method_CreateStack, method_AsBukkitCopy, method_SendPacket, method_GetId, method_Die, method_SetCustomName, method_SetCustomNameVisible, method_SetInvisible, method_SetGravity, method_GetHandle;
    ...
    public static void loadMethods() {
        try {
            method_asNMSCopy = ItemReflection.getClass_CraftItemStack().getMethod("asNMSCopy", org.bukkit.inventory.ItemStack.class);
            method_SaveItem = ItemReflection.getClass_ItemStack().getMethod("save", ItemReflection.getClass_NBTTagCompound());
            method_Add = ItemReflection.getClass_NBTTagList().getMethod("add", ItemReflection.getClass_NBTBase());
            method_Save = ItemReflection.getClass_NBTCompressedStreamTools().getMethod("a", ItemReflection.getClass_NBTTagCompound(), DataOutput.class);
            method_A = ItemReflection.getClass_NBTCompressedStreamTools().getMethod("a", DataInputStream.class);
            method_CreateStack = ItemReflection.getClass_ItemStack().getMethod("createStack", ItemReflection.getClass_NBTTagCompound());
            method_AsBukkitCopy = ItemReflection.getClass_CraftItemStack().getMethod("asBukkitCopy", ItemReflection.getClass_ItemStack());
            //Hologram methods
            method_SendPacket = ItemReflection.getClass_PlayerConnection().getMethod("sendPacket", ItemReflection.getClass_Packet());
            method_GetId = ItemReflection.getClass_Entity().getMethod("getId");
            method_Die = ItemReflection.getClass_Entity().getMethod("die");
            method_SetCustomName = ItemReflection.getClass_EntityArmorStand().getMethod("setCustomName", String.class);
            method_SetCustomNameVisible = ItemReflection.getClass_EntityArmorStand().getMethod("setCustomNameVisible", boolean.class);
            method_SetInvisible = ItemReflection.getClass_EntityArmorStand().getMethod("setInvisible", boolean.class);
            method_SetGravity = ItemReflection.getClass_EntityArmorStand().getMethod("setGravity", boolean.class);
            method_GetHandle = ItemReflection.getClass_CraftWorld().getMethod("getHandle");
        } catch (Throwable ex) {
            ex.printStackTrace();
        }
    }
