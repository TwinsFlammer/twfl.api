package br.com.twinsflammer.api.spigot.nametag.data;

import br.com.twinsflammer.api.spigot.nametag.manager.NametagManager;
import br.com.twinsflammer.api.spigot.nametag.packets.VersionChecker;
...
public class FakeTeam {
    ...
    public FakeTeam(String prefix, String suffix, int sortPriority, boolean playerTag) {
		...
		if(VersionChecker.getBukkitVersion() == VersionChecker.BukkitVersion.v1_13_R1) {
        	...
        	this.name = this.name.length() > 128 ? this.name.substring(0, 128) : this.name;
        } else if(VersionChecker.getBukkitVersion() == VersionChecker.BukkitVersion.v1_14_R2) {
            this.name = this.name.length() > 128 ? this.name.substring(0, 128) : this.name;
		} else if(VersionChecker.getBukkitVersion() == VersionChecker.BukkitVersion.v1_15_R1) {
        	this.name = this.name.length() > 128 ? this.name.substring(0, 128) : this.name;
		} else if(VersionChecker.getBukkitVersion() == VersionChecker.BukkitVersion.v1_15_R2) {
        	this.name = this.name.length() > 128 ? this.name.substring(0, 128) : this.name;
		} else {
